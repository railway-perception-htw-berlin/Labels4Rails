...
You want to visualize the annotations, create new annotations or generate segmentations masks of track annotations and YOLO Scheme labels of the switches.

1. Clone this GIT-repository:
   - `git clone https://github.com/railway-perception-htw-berlin/Labels4Rails.git`
...
- [mandatory] `camera/camera.yaml` contains the camera extrinsic. If there are no camera extrinsics for a dataset, the [camera file](https://github.com/railway-perception-htw-berlin/L4R_NLB/tree/main/L4R_NLB_winter/camera) from the L4R_NLB dataset can be used instead.
...
## Label Tracks
   ...
   - ego track annotations can be generated automatically by clicking the `Auto` button. Existing ego track annotations will be overwritten by the newly created annotations.
...
## Label Switches
   ...
   - if all tracks in the scene are already annotated, switches can be generated automatically by clicking the `Auto` button. The automatically generated switch annotations are only based on the track labels. If the switch direction cannot actually be derived from the image, the direction should be changed manually as described above.
...
## Label Tags
   ...
   - An experimental feature for automatic tag generation is available in the 'autotag' branch. The tags `curve` and `straight` are set based on the geometry of track annotations. In the tag groups `environment`, `light`, `Weather`, and `Time of Day` the most likely tag is selected using an LLM.
